...
def corners4_to_mesh2(corners, color=[255,0,0]):
  ...
  n = corners.shape[0]
  triangles = np.array([[[0,1,2], [2,3,0]]],dtype=np.int32) # [1,2,3]
...
def cut_points_roof(points, keep_rate=POINTS_KEEP_RATE, sample_rate=POINTS_SAMPLE_RATE):
  ...
  threshold = z_min + (z_max - z_min) * keep_rate -0.1
...
class Bbox3D():
  ...
  def draw_lines_open3d(lines, color=[0,0,0], show=False):
    ...
    line_set.lines = open3d.utility.Vector2iVector(lines_vids)
  ...
  def define_wall_direction(box, up_axis, yx_zb, check_thickness):
    ...
    assert box.shape == (7,)
    assert up_axis == 'Z'
    assert yx_zb == True, "the rule for yx_zb is different"
    if up_axis == 'Y':
