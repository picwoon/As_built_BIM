...
def preprocess_cfr(ceilings_org, walls_org, obj):
  ...
  #Bbox3D.draw_bboxes(walls_org, 'Z', False)    #draw 1
  ...
  for c in range(cn):
    ...
    if Debug and 1:
          ...
          Bbox3D.draw_bboxes_mesh(box_show, 'Z', False)  #draw mesh 2
    if num_overlap > 1:
      continue
    #if num_overlap >0:
