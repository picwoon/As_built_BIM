...
def preprocess_windows_multy_levels(windows,walls,doors,celings,floors):
  if DEBUG and 1:
    print('after_process')
    #show_all([windows,walls,doors,celings,floors])
    show_all([windows,walls,doors,celings, floors])
  return windows
...
def preprocess_windows(windows0, walls):
  ...
  if DEBUG and 0:
      print('input')
      show_all([windows0,walls])#show 1
  #Bbox3D.draw_bboxes(walls, 'Z', False)
  #Bbox3D.draw_bboxes(windows0, 'Z', False)
  #print(f'windows0: \n{windows0}')
  if len(windows0) == 0:
    return windows0
  windows1 = Bbox3D.define_walls_direction(windows0, 'Z', yx_zb=True)
  #print(f'windows1: \n{windows0}')
  ...
  if DEBUG and False:
    print('out')
    show_all([windows1,walls]) #show 2
  return windows1
...
def show_all(boxes_ls):
  ...
  if boxes.shape[0]==0:
    return
  Bbox3D.draw_bboxes(boxes, 'Z', is_yx_zb=False)
