...
def preprocess_doors_multy_levels(doors,walls,door_thickness = 0.18):
  if DEBUG:
    show_all([doors,walls])
  return doors
...
def preprocess_doors(doors0, walls, door_thickness=0.18):
  ...
  if DEBUG and fail_match_door_ids.shape[0]>0:
    ...
    show_all([doors0[fail_match_door_ids], walls])       #showing
  ...
  if DEBUG:
    show_all([doors_new, walls]) #show doors
  return doors_new
