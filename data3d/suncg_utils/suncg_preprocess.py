...
from wall_preprocessing import preprocess_walls, preprocess_walls_multy_levels
from window_preprocessing import preprocess_windows,preprocess_windows_multy_levels
from door_preprocessing import preprocess_doors,preprocess_doors_multy_levels
...
def get_part_bbox(vertices, triangle, triangle_norms, name=''):
  ...
  ### got the bbox
  xc,yc,zc = centroid
  bbox = np.array([xc, yc, zc, x_size, y_size, z_size, yaw])#get bboxes change the xy
...
def gen_bbox(house_fn):
    ...
    if box_intact and (not always_gen_bbox):
      ...
      return
    with open(house_fn) as f:#house_fn是house的长串目录
      house = json.loads(f.read())#把house.json文件解码成python对象
    ...
    Temp = 1
    #aa equals wall , bb equals windows , cc equals door , dd equals ceiling , ee equals floor, all of them are in the same level
    aa = []
    bb = []
    cc = []
    dd = []
    ...
    for level in house['levels']:
      ...
      if 'bbox' not in level:
        continue
      bboxes['level' + str(Temp)].append(Bbox3D.bbox_from_minmax(level['bbox']))
      Temp += 1
      nodes = level['nodes']
      for node in nodes:
        node_type = node['type']
      ...
      if bboxes['wall'].shape[0]:
        ...
        aa.append(np.empty((0,7),dtype=np.float64))
      if bboxes['window'].shape[0]:
        bb.append(bboxes['window'][:])
      else:
        bb.append(np.empty((0,7),dtype=np.float64))
      if bboxes['door'].shape[0]:
        cc.append(bboxes['door'][:])
      else:
        cc.append(np.empty((0,7),dtype=np.float64))
      if bboxes['ceiling'].shape[0]:
        dd.append(bboxes['ceiling'][:])
      else:
        dd.append(np.empty((0,7),dtype = np.float64))
      if bboxes['floor'].shape[0]:
...
def gen_pcl(house_fn):
    ...
    for i,depth_fn in enumerate(depth_fns):
      ...
      if gen_ply_each_image:
        pcd = open3d.PointCloud()
        pcd.points = open3d.utility.Vector3dVector(pcl_i)
    ...
    pcd.points = open3d.utility.Vector3dVector(pcls_all[:,0:3])
    pcd.colors = open3d.utility.Vector3dVector(pcls_all[:,3:6])
    pcd = open3d.geometry.PointCloud.voxel_down_sample(pcd, voxel_size=0.02)
    new_num = np.asarray(pcd.points).shape[0]
    print(f'new point num: {new_num/1000.0} K')
    open3d.io.write_point_cloud(pcl_fn, pcd)
    #open3d.draw_geometries([pcd])
...
def save_cam_ply(cam_fn=None, show=False, with_walls=True, with_pcl=True):
  ...
  # save cam pos as ply
  pcd = open3d.geometry.PointCloud()
  pcd.points = open3d.utility.Vector3dVector(cam_pos[:,0:3])
  cam_ply_fn = cam_fn+'_pos.ply'
  open3d.io.write_point_cloud(cam_ply_fn, pcd)
  #open3d.draw_geometries([pcd])
